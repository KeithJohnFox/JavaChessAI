...
*/
  ...
  private Stack getWhitePawnSquares(int x, int y, String piece) {
    ...
    //First move allow a 1 or 2 square move
    for (int i = 1; i < 3; i++) {
      int tmpy = y + i;
      int tmpx = x;
      if (!(tmpy > 7)) {
        if (y == 1) {
          ...
          move1 = new Move(startingSquare, tmp);
          getPiecePresent(moves, move1, tmp);
        }
      }
    }
    //Execute if pawn has made first move, only allow one square move
    for (int i = 1; i < 2; i++) {
      int tmpy = y + 1;
      int tmpx = x;
      if (!(tmpy > 7)) {
        Square tmp = new Square(tmpx, tmpy, piece);
        move2 = new Move(startingSquare, tmp);
        getPiecePresent(moves, move2, tmp);
      }
      if(tmpy == 7){
        pawnToQueen = true;
      }
    ...
    //Execute to take a piece on the right diagonal
    for (int i = 1; i < 2; i++) {
      int tmpy = y + i;
      int tmpx = x + i;
      if (!(tmpy > 7 || tmpy < 0 || tmpx < 0 || tmpx > 7)) {
        Square tmp = new Square(tmpx, tmpy, piece);
        move3 = new Move(startingSquare, tmp);
        getOpponent(moves, move3, tmp);
      }
      if(tmpy == 7){
        pawnToQueen = true;
      }
    ...
    //Execute to take piece on the left diagonal
    for (int i = 1; i < 2; i++) {
      int tmpy = y + i;
      int tmpx = x - i;
      if (!(tmpy > 7 || tmpy < 0 || tmpx < 0 || tmpx > 7)) {
        Square tmp = new Square(tmpx, tmpy, piece);
        move4 = new Move(startingSquare, tmp);
        getOpponent(moves, move4, tmp);
  ...
  private void getPiecePresent(Stack moves, Move validMove, Square tmp){
...
a
  ...
  */
  ...
  /*
    ...
    can move in a diagonal moment. There are essentially four different directions from a single
...
//          if((currentYPos+20)/75) == 7){
//
//          }

//          if(pawnToQueen){
//
//            int location = 56 + (landingPoint.getXC() / 75);
...
//          }
      ...
      //Code executed for NextbestMove
      if(selectedAI == 1 ){
       System.out.println("=============================================================");
       Border redBorder = BorderFactory.createLineBorder(Color.RED, 3);
 ...
 */
  ...
  */
  public static void main(String[] args) {
    ...
    // store AI mode in selectAI var to run correct AI agent
    selectedAI = n;
    frame.makeAIMove();
  }
}
